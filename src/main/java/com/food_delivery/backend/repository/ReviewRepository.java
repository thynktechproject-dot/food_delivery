...
public interface ReviewRepository extends JpaRepository<Review, Long> {
	...
	boolean existsByUserIdAndReviewTypeAndTargetIdAndOrderId(Long userId, ReviewType reviewType, Long targetId,
			Long orderId);

	/**
	 * Find a specific review by user + order + type + target (for update/delete
	 * guard).
	 */
	Optional<Review> findByUserIdAndReviewTypeAndTargetIdAndOrderId(Long userId, ReviewType reviewType, Long targetId,
			Long orderId);
	...
	@Query("""
			    ...
			    FROM Review r
			    WHERE r.reviewType = 'RESTAURANT'
			    AND r.targetId = :restaurantId
			    AND r.createdAt >= :startOfDay
			    AND r.createdAt < :endOfDay
			""")
	Double getTodayAverageRating(Long restaurantId, LocalDateTime startOfDay, LocalDateTime endOfDay);
}