...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderResponse placeOrder(Long userId) {
        ...
        restaurantRepository.findById(cart.getRestaurantId())
                .filter(restaurant -> restaurant.isApproved() && restaurant.isActive())
    ...
    public Page<OrderResponse> getOrdersByRestaurantOwner(Long ownerId, int page, int size) {
        Long restaurantId = getOwnerRestaurantId(ownerId);
        return orderRepository.findByRestaurantId(restaurantId, ordersPageable(page, size))
                .map(this::toResponse);
    }
    ...
    public Page<OrderResponse> getOrdersByDeliveryAgentId(Long deliveryAgentId, int page, int size) {
        return orderRepository.findByDeliveryAgentId(deliveryAgentId, ordersPageable(page, size))
                .map(this::toResponse);
    }
    ...
    private void validateActorCanManageOrder(Order order, User actor, OrderStatus newStatus) {
        ...
        if (actor.getRole() == Role.RESTAURANT_OWNER) {
            boolean ownsRestaurant = restaurantRepository.findByIdAndOwnerIdAndActiveTrue(order.getRestaurantId(), actor.getId()).isPresent();
            if (!ownsRestaurant) {
    ...
    private void validateTransition(Order order, User actor, OrderStatus newStatus) {
        ...
        if (actor.getRole() == Role.RESTAURANT_OWNER) {
            if (currentStatus == OrderStatus.PAID && (newStatus == OrderStatus.CONFIRMED || newStatus == OrderStatus.CANCELLED)) {
                return;
            }
        ...
        if (actor.getRole() == Role.DELIVERY_AGENT) {
            ...
            throw new BadRequestException("Delivery agents can only move assigned orders to OUT_FOR_DELIVERY or DELIVERED");
        }
        ...
        if (actor.getRole() == Role.ADMIN) {
            ...
            if (currentStatus == OrderStatus.CONFIRMED && (newStatus == OrderStatus.PREPARING || newStatus == OrderStatus.CANCELLED)) {
                return;
            }
            if (currentStatus == OrderStatus.PREPARING && (newStatus == OrderStatus.OUT_FOR_DELIVERY || newStatus == OrderStatus.CANCELLED)) {
                return;
            }
            if (currentStatus == OrderStatus.OUT_FOR_DELIVERY && newStatus == OrderStatus.DELIVERED) {
                return;
